...
class CategoryCore extends ObjectModel
{
    ...
    public function delete()
    {
        ...
        Hook::exec('actionCategoryDelete', array('category' => $this, 'deleted_children' => $deletedChildren));
    ...
    public function deleteImage3($force_delete = false)
    {
        if (!$this->id)
            return false;
        ...
        if ($force_delete || !$this->hasMultishopEntries())
        {
            ...
            if ($this->image_dir)
            {
                if (file_exists($this->image_dir.$this->id.'_third.'.$this->image_format)
                    && !unlink($this->image_dir.$this->id.'_third.'.$this->image_format))
                    return false;
            }
            if (file_exists(_PS_TMP_IMG_DIR_.$this->def['table'].'_'.$this->id.'_third.'.$this->image_format)
                && !unlink(_PS_TMP_IMG_DIR_.$this->def['table'].'_'.$this->id.'_third.'.$this->image_format))
                return false;
            if (file_exists(_PS_TMP_IMG_DIR_.$this->def['table'].'_mini_'.$this->id.'_third.'.$this->image_format)
                && !unlink(_PS_TMP_IMG_DIR_.$this->def['table'].'_mini_'.$this->id.'_third.'.$this->image_format))
                return false;

            $types = ImageType::getImagesTypes();
            foreach ($types as $image_type)
                if (file_exists($this->image_dir.$this->id.'_third-'.stripslashes($image_type['name']).'.'.$this->image_format)
                && !unlink($this->image_dir.$this->id.'_third-'.stripslashes($image_type['name']).'.'.$this->image_format))
                    return false;
        }
        return true;
    }
    public function deleteImage4($force_delete = false)
    {
        if (!$this->id)
            return false;
        ...
        if ($force_delete || !$this->hasMultishopEntries())
        {
            ...
            if ($this->image_dir)
            {
                if (file_exists($this->image_dir.$this->id.'_fourth.'.$this->image_format)
                    && !unlink($this->image_dir.$this->id.'_fourth.'.$this->image_format))
                    return false;
            }
            if (file_exists(_PS_TMP_IMG_DIR_.$this->def['table'].'_'.$this->id.'_fourth.'.$this->image_format)
                && !unlink(_PS_TMP_IMG_DIR_.$this->def['table'].'_'.$this->id.'_fourth.'.$this->image_format))
                return false;
            if (file_exists(_PS_TMP_IMG_DIR_.$this->def['table'].'_mini_'.$this->id.'_fourth.'.$this->image_format)
                && !unlink(_PS_TMP_IMG_DIR_.$this->def['table'].'_mini_'.$this->id.'_fourth.'.$this->image_format))
                return false;

            $types = ImageType::getImagesTypes();
            foreach ($types as $image_type)
                if (file_exists($this->image_dir.$this->id.'_fourth-'.stripslashes($image_type['name']).'.'.$this->image_format)
                && !unlink($this->image_dir.$this->id.'_fourth-'.stripslashes($image_type['name']).'.'.$this->image_format))
                    return false;
        }
        return true;
    }
    ...
    public static function duplicateProductCategories($idOld, $idNew)
    {
        ...
        if ($result) {
            foreach ($result as $i) {
                $row[] = '('.implode(', ', array((int) $idNew, $i['id_category'], '(SELECT tmp.max + 1 FROM (
    ...
    */
    ...
    public static function getLastPosition($idCategoryParent, $idShop)
    {
        if ((int) Db::getInstance()->getValue('
				...
				WHERE c.`id_parent` = '.(int) $idCategoryParent) === 1) {
            return 0;
        } else {
